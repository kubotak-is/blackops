...
Application Rootに`migrations/`が存在する場合、`database:status`と`database:migrate`は`App\Migrations`をFramework Migrationと同じDoctrine Migration Configurationへ追加する。Directoryが存在しない場合はFramework Migrationだけを扱い、空Directoryを要求しない。
...
更新対象の公式EntrypointはProject Rootの`blackops`だけとし、旧`bin/blackops`の維持は検証しない。
