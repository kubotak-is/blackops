...
/concepts/...         Why BlackOps、Core Concepts、Operation Lifecycle
/getting-started/...  Installation、Quickstart、Tutorial、Directory Structure、Local Runtime
/operations/...       Operation、Value、Outcome、Generator、Validation
/execution/...        HTTP、Inline、Deferred、Worker、Context
/database/...         Migration、Outcome Retrieval、Retention
/testing/...          Application Testの入口
/deployment/...       Production Worker運用の入口
/security/...         Security責任分界とSensitive Data
/troubleshooting/...  症状別FAQ
/releases/...         Current Status、Stable／main差、既知制約
/reference/...        Core API、Attribute、Configuration、BlackOps CLI、Application Bootstrap、Glossary
```

Navigationは利用者がApplicationを構築する順序を優先する。Getting StartedはInstallation、Quickstart、Tutorial、Directory Structure、Local Runtimeの順とする。説明はOverview、TutorialはGetting Started、目的別How-toはOperations以降、引く情報はReferenceへ分離する。各Pageは現在のPublic APIだけで完結し、Internal Namespace、Orchestration ID、Acceptance Hash、Framework実装手順を利用者向け説明として要求しない。

Sidebar Labelは`Execution & Workers`と`Data & Retention`を使用する。ReferenceへTroubleshooting、Security、Current Statusを混在させず、それぞれ独立Sectionへ配置する。TestingとDeploymentは入口Pageから開始し、未提供のTesting APIやProduction Integrationを実装済みとして説明しない。
