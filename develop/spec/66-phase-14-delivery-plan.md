...
- `config/logging.php`のBuilt-in JSONL Backend、限定Stream、厳密Validation、Best-effort Runtime Failureを実装する
...
- Logging Disable Switch、Custom Backend Public Selection、暗黙Fallbackを追加しない
...
- [x] Local Viewerが既定無効、明示起動、Loopback限定、Random Token必須、Read-onlyである
- [x] HTTP、Log、Terminal、ViewerへCredential、Raw Value、Actor ID、Exception／Dead Letter Messageを出さない
- [x] Production Log Sink／Retention／AlertとFramework相関の責任境界が明文化される
- [x] Public PHP Query API、Status／Outcome HTTP API、Tenant分離、OpenTelemetryをPhase 14へ追加しない
- [x] Quickstart、Skeleton、Guide、Consumer E2EがPublic CLI／Viewer Contractを再現する
...
- Phase 20: Canonical Raw Access、暗号化、Tenant分離、特権Audit、OpenTelemetry、Metric、Remote Exporter、Health／Readiness。
...
- Logging Decision: [D099 Production Logging Configuration](../decisions/099-production-logging-configuration.md)
