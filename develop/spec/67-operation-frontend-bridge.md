...
- Value Fieldごとの名前、PHP Type、Nullable、Required、Binding Source、Transport Name、Sensitive有無、Validation Rule Metadata
- Outcome Class、成功Mode、Public Fieldごとの名前とPHP Type
...
| `bool` | `boolean` |
| Nullable | `T \| null` |
| Constructor Defaultあり | Optional Property |
| `void`／`EmptyOutcome` | `undefined` |
...
PHP Native TypeだけからTypeScript Contractを確定できない型を`any`または`unknown`へ暗黙変換しない。Typed List／Map、Nested DTO、Enum、Date／Timeは、PHP側の表現とHTTP Binderを同時に決める後続Decisionで追加する。
...
Global Mutable Client ConfigurationはSSR Request間のCredential Leakを招くため提供しない。Application共通設定が必要な場合はApplicationが薄いWrapperを作る。
...
Generated RuntimeはStatus、Content-Type、JSON Object、Discriminant、既知共通Field、Operation固有Outcome Scalar Fieldを検証する。不正または未知のResponse Shapeを成功型へCastせず`unexpected_response`へ変換する。Raw Body、Credential、Exception DetailをTransport Errorへ含めない。
...
- Typed List／Map、Nested DTO、Enum、Date／Time、Upload／Stream、Custom Responderは後続Decisionへ送る
