...
- `EphemeralOutcome`だけはCredential Propertyの`#[Sensitive]`を必須とし、直接`fetch()`のResponse Typeへ名前と型だけを生成する。値、Default、ExampleをArtifactへ含めない
- Ephemeral Operation Objectは`.status()`／`.wait()`を公開しない。`.fetch()`／`.toRequest()`／`.url()`は通常のHTTP Operationと同じ安全なBindingを使う
