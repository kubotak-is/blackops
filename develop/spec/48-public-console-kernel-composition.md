...
Application-aware BuildはSymfony `#[AsCommand]` Metadataと、Operation `#[ConsoleCommand]` MetadataをConstructor実行なしで抽出し、Schema 2のPHP Array Command ManifestへAtomicに保存する。`commands`はApplication Command、`operation_commands`はOperation Type／Definition／Value／Outcome／Strategyと順序付きScalar Optionを保持する。Command ClassはApplication Service Provider適用後にAutowired Public Serviceとして登録し、同じService IDのApplication定義を優先する。Unresolved Constructor Dependency、Framework／Application／Operation／Explicit NameまたはAlias衝突はBuild Errorとする。

新ProcessのConsole KernelはValidで同じBuild IDのCommand ManifestからSymfony `LazyCommand`と軽量Operation Command Adapterを登録する。Global `list`はManifest Metadataだけを使い、Operation HelpもContainer／Database／Actor Providerを解決しない。実行時は同じBuild IDのOperation Manifest／Compiled Containerと一致を再検証し、共通Operation Runtimeを構成する。Schema 1／Missing／Invalid／Stale Manifestでは両Application Command群を登録せず、Framework `build:compile`を含むBuilt-in Commandを維持する。Runtime Source Scan、Attribute Reflection Fallback、`new $class()`は行わない。
...
Main Worker StorageとReceiverは`framework.connection`のDBAL Connectionを共有する。DatabaseManagerとDefault ConnectionはHandler解決前にCompiled Containerへ設定する。Heartbeat ReceiverはFramework Connectionと同じParameterから生成したApplication非公開の別Connectionを使う。同じPCNTL Signal Heartbeat InstanceをClaim Execution GuardとWorker Signal Runtimeへ渡す。
...
Retention Runtimeは`framework.connection`からPlanner、Purge Audit、Transport Payload Tombstone、Outcome／Dead Letter／Journal Delete Serviceを構成する。Schedulerは同じPurge ServiceとPolicyを使うRetention Maintenance Taskを登録する。
