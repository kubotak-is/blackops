...
Build時Containerは`Psr\Log\LoggerInterface`をSynthetic Runtime Serviceとして定義する。HTTP RuntimeとDeferred Worker Runtimeは同じOperation境界で共有する`ExecutionScopeProvider`と`ExecutionScopedLogger`を構成し、そのLogger InstanceをCompiled Containerへ設定する。Handler、Authorization Policy、Application Serviceは`LoggerInterface`をConstructor Injectionできる。
...
Operation ScopeはStackとして扱い、Nested Operation終了後に親Scopeを復元する。HTTP Request間とDeferred Attempt間にScopeを残さず、Operation外のLogへ架空のOperation Contextを付与しない。

Application HTTP Runtimeは、内部Error BoundaryがThrowableをSafe HTTP 500へ変換した場合も失敗Invocationとして扱う。ResponseをClientへ返す前に生成済みApplication ConnectionをすべてCloseし、次RequestでHealth Checkと再接続を行う。500 Responseを成功InvocationとしてConnection再利用判定へ送らない。
