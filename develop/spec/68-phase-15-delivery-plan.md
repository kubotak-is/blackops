...
- [x] Operation／HTTP／Frontend Manifestが同じApplication Build IDを持つ
- [x] `#[Route]`を持つ全HTTP OperationだけをFrontend Contractへ含める
- [x] Unsupported Type、Sensitive Outcome、Module／Export CollisionをBuild時に拒否する
- [x] `frontend:generate`が安全かつ決定的にTypeScript ESM Treeを生成する
- [x] Generated Operation Objectが`.fetch()`、`.toRequest()`、`.url()`、Readonly Metadataを提供する
- [x] Path／Query／Header／Body BindingがHTTP Runtimeと一致する
- [x] Inline／Void／Deferred／Protocol／Rejected／Validation／Internal／Transport Resultを型付きで区別する
- [x] BrowserとSSR／Node／TestのFetch境界をFramework-neutralに提供する
- [x] Sensitive Input値、Credential、Raw Error BodyをGenerated Artifact／Resultへ出さない
- [x] `frontend:check`とTypeScript Compile／Runtime TestがCIで成功する
- [x] Quickstart、Skeleton、Guide、Website、Framework Update、Consumer E2Eが同期する
- [x] Callable／Thenable、Global Mutable Client、Retry、Polling、Frontend Framework Adapter、Vite Pluginを追加しない
- [x] Full PHP／Consumer／Frontend／Website Quality Gateが成功する

## Deferred Scope

- Phase 16: Deferred Status／Outcome APIとGenerated Polling連携
- Phase 19: Idempotency、Retry Delivery、OutboxとClient Retry Policyの責任境界
- Phase 20: Tenant、Canonical Raw Access、Encryption、OpenTelemetry、Metric
