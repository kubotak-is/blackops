...
- [x] Optional Key付きInline／Deferred Requestが同じActor／Inputで同じOperationへ収束する
- [x] Current Authentication／Authorizationが重複Requestでも毎回評価される
- [x] Conflict／In-progress／Expired／UnsupportedがStable Failure Matrixを満たす
- [x] Raw Key／Sensitive Input／Credentialが永続化・Journal・Log・Diagnosticsへ残らない
- [x] Idempotency Recordが独立Retention／Hold／Auditへ統合される
- [x] 同一Named Connection内でApplication MutationとOutbox Insertが原子的にCommit／Rollbackする
- [x] RelayがLease／Heartbeat／Fencing／Retry／Dead Letterをat-least-onceで実行する
- [x] Dead Letter再開、Operation Replay、Observer ReplayのIdentity規則が分離される
- [x] Community Boardの二重DigestとComment Notification JourneyがBrowserから完走する
- [x] Framework／Consumer／Frontend／Website Full Gateが成功する
