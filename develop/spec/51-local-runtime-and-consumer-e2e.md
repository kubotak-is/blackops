...
Official FrankenPHP 1／PHP 8.5 Debian ImageへPDO PostgreSQL等の必要Extensionを追加する。Application Directoryを `/app` へMountし、Caddyfileから `public/index.php` をClassic Modeで実行する。
...
- `http` はHealthy PostgreSQLを待つが、Schema MigrationやArtifact Buildを実行しない
...
7. Default `postgres`／`http` 起動
8. `GET /welcome` が200／Expected JSON
9. Welcome Sensitive Header Raw値がJSONLに存在せずMask値が存在
10. `POST /reports` が202／Operation ID
11. Worker 1回目でRetry Scheduled
12. Due Time後のWorker再実行でCompleted
13. PostgreSQL Operation StateがCompleted
14. Outcome Rowが存在し、Encoded Outcomeが保存される
15. Retention PlanとPurge Dry Runが成功
16. Scheduler／Purge Confirmは実行しない
