...
- 第一引数Classは `#[Accepts]` のValue Classと完全一致
- 第二引数がある場合はNamed `ExecutionContext`、Required、Non-reference、Non-variadic
- Native Return TypeはNamed `OperationResult`
...
5. Resultが `OperationResult` であることを確認する
...
- Typed SignatureのValueがManifest Valueと一致
...
- Decision: [D074 Typed Self-handled Operation Signature](../decisions/074-typed-self-handled-operation-signature.md)
