...
初期ScopeではORMとRepository基底Classを標準化せず、Connection、DI、Transaction境界を提供する。Operation Transactionは固定Lifecycle、一般Service TransactionはCompiled Method Interceptorとし、汎用Operation Middlewareは導入しない。After Commitは同期Best-effort、Transactional OutboxはPhase 17とする。
