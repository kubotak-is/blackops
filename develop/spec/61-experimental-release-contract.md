...
`1.1.0`ではProject Root `blackops`を唯一の公式Entrypointとし、旧`bin/blackops`を互換対象にしない。BlackOps CLIの旧`blackops:*` Aliasは削除し、PrefixなしCanonical CommandだけをFramework所有名として予約する。
