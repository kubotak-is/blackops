...
Status: Decided (Phase 21 complete)

Phase 21 replaced Ray.Aop with the Framework-owned proxy contract in Specification 101. Production work was split into six non-overlapping slices and P21-007 accepted the removal gate; Framework is now the sole active profile.

## Dependency order

```text
P21-002 Contract / metadata / signature / ownership guard (Accepted)
  -> P21-003 Generator / artifact / manifest / drift (Accepted)
    -> P21-004 Symfony DI preservation (Accepted)
      -> P21-005 Transaction / AfterCommit runtime and no-double-intercept (Accepted)
        -> P21-006 Ray/framework compatibility and migration (Accepted)
          -> P21-007 Ray removal / package export / closeout (Accepted)
```

All Phase 21 Tasks are accepted and their Reports／STATE checkpoints are synchronized.
...
| P21-006 | Central Application-aware profile migration, manifest-aware RuntimeContainerDumper integration, Framework/Ray compatibility evidence, previous-build rollback, consumer package/export matrix, and accepted Ray removal manifest | P21-005 | Framework support matrix, bounded legacy evidence, Application command/help/Dumper/docs wiring, migration/rollback evidence, no unproxied fallback, clean Consumer package checks, reviewed removal manifest |
...
P21-006 recorded two compatibility-period exceptions: Ray 2.20.0 could not compile an attributed `never` method on PHP 8.5 and dropped extra named variadic values. P21-007 removed Ray and both exceptions; the Framework contract continues to support both signatures.

## Rollback invariant

P21-002 through P21-006 kept the legacy Ray profile selectable. P21-007 accepted its removal. Rollback is now a complete previous Framework Container＋manifest＋artifact directory selection; no fallback or profile stacking is permitted.
