...
| P21-006 | Central Application-aware `build:compile --proxy-profile=ray|framework` selector (default `ray`), manifest-aware RuntimeContainerDumper integration, mutually exclusive profiles, application migration, golden compatibility fixtures, previous-build rollback, consumer package/export matrix, and accepted Ray removal manifest | P21-005 | Both-mode fixture parity except the explicit legacy-Ray `never` compilation and named-variadic forwarding limitations; Framework support for both and all other shared rows, Application command/help/Dumper/docs wiring, migration/rollback evidence, no unproxied fallback, clean Consumer package checks, reviewed removal manifest |
...
P21-006 records two compatibility-period exceptions: Ray 2.20.0 cannot compile an attributed `never` method on PHP 8.5 because its generated proxy returns, and its `func_get_args()` invocation path drops extra named variadic values. The Framework contract continues to support both signatures. Both exceptions expire with the Ray profile removal in P21-007 and do not authorize fallback, a Framework contract reduction, or an exception for any other Matrix row.
