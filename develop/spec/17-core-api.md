...
UUIDv7をApplication Infrastructureへ提供する`BlackOps\Identifier\Uuidv7Generator`は、Vendor UUID型を露出しないPHP Public APIとして扱う。

IdempotencyのPHP Public Valueは`BlackOps\Idempotency\IdempotencyKey`と
`BlackOps\Idempotency\IdempotencyKeyHash`だけである。KeyはPrintable ASCIIのShapeだけを検証し、
Raw Valueを返さず`hash(): IdempotencyKeyHash`だけを公開する。HashはVersion付きSHA-256 Digestを
保持し、Internal Scope／Fingerprintの比較はConstant-timeで行う。
