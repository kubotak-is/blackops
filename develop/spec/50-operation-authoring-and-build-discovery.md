...
標準形はOperation Definition自身がHandler Contractを実装する。
...
Self-handled Operationは `#[HandledBy]` を指定しない。CompilerはHandler MetadataへDefinition Class自身を設定する。
...
- `OperationHandler` を実装し `#[HandledBy]` がない: Definition自身をHandlerとする
...
WelcomeとReportのSeparate Handler Fileを削除し、各Operation Definitionへ `handle()` を移す。BootstrapはProviderを明示登録せず、Configurationだけを読み込む。
