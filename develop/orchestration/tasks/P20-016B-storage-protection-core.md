...
- New third-party Composer Packageを追加しない
- Runtime必須の`ext-sodium` Platform RequirementをComposer Metadataへ明示する
- WorkerはReview前にCommitしない

## Acceptance Criteria

- [x] Public Key Provider ContractがApplication-wide／Per-tenant／Per-purposeを表現できる
- [x] Version 1 EnvelopeがKnown-answerとRandomized Payloadでround-tripする
- [x] Empty／binary／large Payloadを扱える
- [x] Wrong Tenant／Purpose／Record／Operation／KeyでDecryptを拒否する
- [x] Header／Length／Nonce／Ciphertext／Tagの改ざんを拒否する
- [x] Unknown Version／Algorithm／KeyをSafe Failureにする
- [x] Key MaterialがArtifact／Exception／Log／Journal／Test Failureへ露出しない
- [x] Provider BindingがClassic／Worker／CLI Compositionで同じである
- [x] Existing Adapter挙動とFull Suiteを維持する
