...
- `examples/quickstart/bin/setup`（Git executable mode修正のみ）
...
- [x] Committed QuickstartだけからPackage Rootを抽出する
- [x] Split RootのComposer MetadataとExecutableが正しい
- [x] Lock、Vendor、Path Repository、Generated StateをPackage Sourceへ含めない
- [x] Skeleton／Framework `1.0.0` と `^1.0` Constraintが整合する
- [x] 通常Local create-projectが成功する
- [x] Post-create Setup、Lock、Vendor、Autoload Smokeが成功する
- [x] `--no-scripts` create-projectとManual Setupが成功する
- [x] 両InstallがsymlinkではなくCopyで成立する
- [x] Install中にBuild／Migration／Docker Side Effectがない
- [x] Source Tree非変更と一時Resource Cleanupが成立する
- [x] Composer、Architecture／Full Test、Mago、Deptrac、Guardが成功する
