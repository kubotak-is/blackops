...
- New or existing `src/Internal/Outcome/**`
...
If a shared Structured Outcome component needs a different Internal Namespace, keep it under `src/Internal/**` and record the exact ownership in the Report. Do not expose Internal Contract from Public signatures.
...
- New or existing `tests/Internal/Outcome/**`
...
- [ ] `OutcomeData`と`ListOf`がPublic APIとして実装される
- [ ] Supported Structured ShapeをBuild／Runtimeで同じ規則により検証する
- [ ] Unsupported ShapeとCycleをSafeにFail-fastする
- [ ] Frontend Manifest Schema 3とMarker更新が旧Artifactを拒否する
- [ ] Generated TypeScriptがReadonly DTO／ReadonlyArrayとStrict Recursive Decoderを持つ
- [ ] Inline HTTPとDeferred Statusが同じStructured JSONを返す
- [ ] PostgreSQL Outcome Codec Version 2がStructured OutcomeをRound-tripする
- [ ] Version 1／未知Versionを拒否し互換Layerを追加しない
- [ ] Canonical JournalがStructured Completed OutcomeをRound-tripする
- [ ] Sensitive ProjectionがNested DTO／Listを安全に扱う
- [ ] Existing Scalar／Empty OutcomeとGenerated Operation APIが回帰しない
- [ ] OperationValue Input Contractを変更しない
- [ ] Community Board／Quickstart／Skeleton／Website Sourceを変更しない
- [ ] Public GuideとPublic API件数がSourceと一致する
