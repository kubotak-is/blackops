...
- Runtime Handler ResolverだけがCompiled ContainerからInstanceを取得する
...
- [ ] Self-handled OperationをHandler MetadataへCompileできる
- [ ] Required Constructorを持つSelf-handled OperationをRoute／Manifest Compileできる
- [ ] Separate Handler方式が回帰しない
- [ ] Ambiguous／Missing／Invalid Handlerを拒否する
- [ ] Build／ListがDiscovery RootだけでQuickstart Operationを検出する
- [ ] Provider＋Discovery Mergeと重複排除が成立する
- [ ] Buildが全HandlerをContainerへ自動登録する
- [ ] Service ProviderのRepository BindingをSelf-handled Operationへ注入できる
- [ ] RuntimeがSource Discoveryを行わない
- [ ] Quickstartから4 Boilerplate FileとProvider Fluent Callがなくなる
- [ ] Quickstart HTTP／Worker／Retry／Outcome Integrationが成功する
