...
- [x] Digest same-key replayが同じOperation ID／一件のDigestへ収束する
- [x] Digest fresh-key submitが別Operation ID／別Digestになる
- [x] Raw Idempotency KeyがStorage／Journal／Log／Browser Bundleへ残らない
- [x] CommentとOutbox登録が同じTransactionへ参加し、両方向のRollbackが検証される
- [x] Relay停止／再開／Worker／Duplicate Child Deliveryが実PostgreSQLで検証される
- [x] Notification StoreがChild Operation IDで冪等化し、RecipientだけへLatest-firstで返す
- [x] Self-comment、Source削除、Unauthorized Recipientが安全に処理される
- [x] Additive Migration up/down、Fresh Install 11件、Seed 3/3/4/0が成功する
- [x] Server-only BFF、Frontend Unit、Browser二Session Journeyが成功する
- [x] Existing Framework Full PHPUnit、Mago、Deptrac、Frontend、Consumer Gateが回帰しない
- [x] `src/**`、External Delivery、External Publication／Deployへ差分がない
