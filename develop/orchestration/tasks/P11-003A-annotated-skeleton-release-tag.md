...
- [ ] Local Publication Testがannotated tag objectを生成する
- [ ] Local TestがTag Object Type `tag`、所定Message、Peeled Split Commit一致を検証する
- [ ] Workflowがannotated Tag RefをSkeleton RemoteへPushする
- [ ] Existing annotated tagはPeeled Commit一致時だけIdempotent Successとなる
- [ ] Existing annotated tagのPeeled Commit不一致をFail-closedで拒否する
- [ ] New ReleaseのExisting lightweight tagをFail-closedで拒否する
- [ ] Legacy Skeleton `1.0.0` lightweight tagだけは同一Split CommitならImmutable Recoveryとして許可する
- [ ] Legacy `1.0.0` Tagを移動／削除／置換するPathがない
- [ ] Manual DispatchとTag Triggerが同じContractを通る
- [ ] Workflow／Shell Regression Guardが成功する
- [ ] Composer、Mago、PHPUnit、Deptrac、関連Consumer Gateが成功する
- [ ] Internal Documentation、Report、STATEが更新される
...
! rg -n 'force|--force|-f[[:space:]]' .github/workflows/publish-skeleton.yml tests/Consumer/skeleton-publication.sh
