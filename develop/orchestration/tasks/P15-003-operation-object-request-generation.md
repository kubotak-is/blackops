...
Status: Blocked — D101 User Answer Required
...
P15-003の実装前にD101を確定し、Path／Query／Headerの`int`／`float`／`bool`をServerとGenerated Runtimeで同一Contractへ揃える。D101未決定のままFrontend側だけで文字列化またはCastしない。
