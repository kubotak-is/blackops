...
- `tests/Consumer/community-board-foundation.sh`（P17-007で追加済みの正規`digest.server.ts` WrapperをGenerated Import Allowlistへ同期する機械的修正だけ）
- `tests/Consumer/community-board-identity.sh`（同じGenerated Import Allowlistの機械的修正だけ）
...
P17-008 Browser E2E成功後の既存Foundation／Identity Journeyで、Generated Import GuardがP17-007以前の2 Wrapperだけを期待し、P17-007で追加・Commit済みの`digest.server.ts`を許可していない追随漏れを確認した。正規Wrapperは`board.server.ts`、`digest.server.ts`、`operations.server.ts`の3件であり、Production／Frontend Sourceを変えず両Guardの期待値だけを同期することを許可する。それ以外の既存Consumer Assertion変更は許可しない。
...
- [x] Design Read、Dial、Auditに沿う一貫したVisual Languageが全Routeへ適用される
- [x] Reicon SvelteをStatic Importし、別Icon Family／CDN／hand-written SVGを使わない
- [x] Global Token、Light／Dark、Typography、Spacing、Shape、Focusが統一される
- [x] Desktop／MobileでHorizontal OverflowがなくNavigationとFormが利用できる
- [x] Landing、Authentication、Feed、Detail、Form、Digestの全Stateが完成する
- [x] Existing Server Contract、Progressive Enhancement、Test ID、Safe Error境界を維持する
- [x] Keyboard、Focus、Label、Error Association、Contrast、Reduced Motionを検証する
- [x] PlaywrightがRegisterからLogoutまでの実Browser Journeyを完走する
- [x] 主要PageのCritical／Serious Accessibility Violationが0である
- [x] Credential／Internal URLを含まない実画面ScreenshotをCommitする
- [x] Frontend check、Vitest、Build、Browser E2E、既存5 Consumer Journeyが成功する
- [x] Root Mago／PHPUnit／DeptracとQuickstart Guardが成功する
- [x] Runtime／Generated／Browser ArtifactをCleanupする
...
! rg -n '—|–|&mdash;|&ndash;' examples/community-board/frontend/src --glob '*.svelte' --glob '*.ts' --glob '*.css'
git diff --check
