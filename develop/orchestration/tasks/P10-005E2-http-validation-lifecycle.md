...
- `develop/decisions/087-pre-binding-rejection-journal.md`
- `develop/decisions/088-validation-backend.md`
...
- Raw／Sensitive ValueをResponse、Exception、`OperationRejectedData`、Observed Journalへ含めない
- Canonical `OperationReceivedData`はD089どおり再現可能な実Valueを保持する
...
- Binding FailureはSequence 1の`OperationRejected`だけを記録し、`OperationReceived`を作らない
- Value Validation Failureは`OperationReceived`から`OperationRejected`へ遷移する
- BlackOps Validation AttributeをSymfony Validator ConstraintへInternalで変換し、Symfony型をPublic Contractへ露出しない
...
- [ ] Response／Rejected Data／Observed Journal／ExceptionにRaw Sensitive Valueがない
- [ ] Value Validation FailureのCanonical Receivedが実Valueを保持し、既存再現可能性を維持する
