...
| Initial Split Commit／Tag Push | Not executed; Distribution `main`／`1.0.0`は存在しない |
...
PackagistのComposer Metadataで`blackops/framework` Stable `1.0.0`がCommit `279716f904f17be9341f3fdaae30156ab17d8a62`を参照することを確認した。Framework Release TagからSkeletonを生成した後に`blackops/skeleton`を登録する必要がある。
...
- Initial Workflow Run `29226110632`は`/app/vendor does not exist and could not be created`で失敗した。Permission Fixは実装・Local検証済みで、Live Recovery Runが必要である。
- 公開済み`1.0.0` Tagを移動せずMain上の修正版Workflowを使用するManual Recovery Pathは実装済みで、Live Runが必要である。
...
Recovery WorkflowをCommit／Pushし、Mainから`release_version=1.0.0`でManual Dispatchする。Distribution `main`とTagを確認後、生成された`blackops/skeleton`をPackagistへ登録し、P8-004 Remote Create-project Smokeへ進む。
