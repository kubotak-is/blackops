...
- 現行Skeleton Constraint `^1.0`で初回Publicationを成功させるには1.x Tagが必要だが、Task PacketはStable `1.0.0` ReleaseをOut of Scopeとしている。Public Tag作成前に初回Versionのユーザー判断が必要である。
...
初回Stable `1.0.0`を公開するか、Pre-1.0向けにSkeleton ConstraintとPublication Contractを変更するかを決定する。Decision／Specification／Task Packetへ反映後、合意したbare SemVer TagでWorkflowを実行し、Distribution `main`とTagを確認する。生成後の`blackops/skeleton`をPackagistへ登録し、P8-004 Remote Create-project Smokeへ進む。
