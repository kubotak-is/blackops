...
- Initial Workflow Run `29226110632`は`/app/vendor does not exist and could not be created`で失敗した。GitHub Runner UID／GIDとCompose Container Userの不一致を修正する必要がある。
- 公開済み`1.0.0` Tagを移動せずMain上の修正版Workflowを使用するため、既存Tagを明示するManual Recovery Pathが必要である。
...
D079に従い、GitHub Runner UID／GIDをComposeへ渡すPermission Fixと、既存bare SemVer Tagを明示CheckoutするManual Recovery PathをWorkflowへ追加する。修正版をMainから`1.0.0`へ実行し、Distribution `main`とTagを確認する。生成後の`blackops/skeleton`をPackagistへ登録し、P8-004 Remote Create-project Smokeへ進む。
