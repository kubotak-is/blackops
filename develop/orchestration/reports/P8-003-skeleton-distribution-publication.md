...
| --- | --- |
| Main Framework `kubotak-is/blackops` | Created; Public・Admin権限を確認。Local Historyの接続は未実施 |
...
- Main Framework Repository `kubotak-is/blackops`へWorking Repository Historyを接続する必要がある。
- DistributionのDefault Branch `main`は初回Workflow Push後に確認する必要がある。
- Packagist `blackops/skeleton`を`kubotak-is/blackops-skeleton`のGitHub連携へ登録する必要がある。
...
Main Framework RepositoryへLocal Historyを接続・Pushする。続いてPackagist GitHub連携、初回bare SemVer Tag Workflow、Distribution `main`、P8-004 Remote Create-project Smokeを検証する。
