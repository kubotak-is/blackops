...
- `docs/internal/execution-context.md` (new): AttemptContext、ExecutionContext、Internal ExecutionContextFactoryのPublic API、不変条件、Namespace/依存方向、品質検査結果をFramework実装者向けに整理した。
- `docs/internal/README.md` (edit): Planned Topicsへ Execution Context のLinkを追加した。
...
- `develop/STATE.md` (edit): Task StatusをIn Progress→本Report持出しで更新する。Timestampは秒とUTC Offsetを含むISO 8601形式。
- `develop/orchestration/reports/P1-002-execution-context.md` (new): 本Report。
...
Result: No matches. Production CodeとTestのComment／DocBlockから仕様番号、Decision番号、Task番号、develop/TODO.md行参照を排除した。
```
...
- [x] Public APIと不変条件が内部文書へ記録される（`docs/internal/execution-context.md`、`docs/internal/README.md`）。
