...
D108のA／B／Cに従い、literal workaroundを維持してP17-008を先行する。upstreamへのIssue／PRは作成しない。Ray.AopはPhase 20の`#[Transactional]`／`#[AfterCommit]`専用Framework-owned Build-time Proxyで置き換え、Compatibility TestとRemoval Gateの成功後に削除する。
