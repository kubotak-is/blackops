...
UserはQuestion 1からQuestion 7まで、すべてAを選択した。Question 4とQuestion 6を含むSecurity Contractが確定したため、後続SpecificationとProduction Code実装へ進める。

## Decision

[DECISION]

1. `GET /operations/{operationId}`の一つのVersion付きResourceがStatusとTerminal Resultを返す。
2. Public Stateは`accepted`、`running`、`retry_scheduled`、`completed`、`rejected`、`failed`、`dead_lettered`の7種類とする。
3. Deferred受付は202と`Location`／`Retry-After`を返し、Status Resourceは存在する全Stateで200を返す。
4. Status／Outcome参照はApplication必須の専用Query Authorizerで評価し、未登録／DenyはFail-closedにする。Persisted OperationValueでExecution Authorizationを再実行しない。
5. `completed`はOperation固有Typed Outcome、`rejected`はSafe Category／Code、`failed`／`dead_lettered`は安定Public Codeだけを返す。
6. UnknownとUnauthorizedは同じ404へ丸める。Applicationが参照を許可し、Retention Tombstoneが残る場合だけExpiredを410で返す。TombstoneもPurge済みなら404とする。
7. 全Generated Operation Objectへ`.status(operationId, options)`と`.wait(operationId, options)`を追加する。`.fetch()`は202後に自動Pollingしない。
8. `.wait()`はAbort Signalと有限の`maxWaitMilliseconds`を必須とし、`Retry-After`をHintにする。Global Mutable Stateを持たず、Network／Authentication／Unavailable／Expired／Server Errorを無制限Retryしない。
...
- Clientは202のOperation IDから、追加のOutcome Endpointを組み立てずに現在StateとTerminal Resultを取得できる。
- Query AuthorizationはOperation実行時の`#[Authorize]`と別Contractになり、ApplicationはResult Read Policyを明示実装する。
- Internal DiagnosticsとCanonical StoreはそのままPublic DTOにならず、Status専用のSafe Projectionが必要になる。
- Operation IDの存在をDeny Responseから判別できない。Expiredの識別可否はRetention後に認可SubjectとTombstoneを構成できるかに依存する。
- Generated Operation Objectは明示Capability Methodを増やすが、Thenable／Automatic Polling／Global Clientにはならない。
- Status Resource Schema、Public PHP Query Model、PostgreSQL Projection、HTTP Composition、Generated Decoder／Pollingを順序付けて実装する必要がある。
