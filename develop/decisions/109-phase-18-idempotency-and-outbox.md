# D109: Phase 19 Idempotency and Outbox

Status: Decided

## Context

Phase 19はReliability and Deliveryを扱う。確定済みRoadmapには次が含まれる。
...
- BlackOps Boardの同一User／Week Digestは、異なるRequestなら複数Rowを許可する。Phase 19のIdempotencyは同一Requestの再送だけをまとめる。
...
- A: 最初のScopeを`Operation Type ID + authenticated Actor type/id + Idempotency Key`とする。ActorがないRequestでKeyが指定された場合は安全な400で拒否し、Tenant ScopeはPhase 20で追加する
...
同じKeyを別Userが使っても衝突せず、他UserのOperation IDやResultを返さない。Anonymous Global Scopeは推測可能なKeyによる衝突／妨害を起こしやすい。必須Resolverは柔軟だが、最初の利用にProvider実装を要求する。Phase 19はBlackOps Boardの認証済みJourneyを正本にし、TenantとAnonymous IdempotencyはSecurity設計と一緒に拡張する。
...
Phase 19は、次のContractでIdempotency、Transactional Outbox、Relay／Replayを実装する。
...
3. Scopeは`Operation Type ID + authenticated Actor type/id + Idempotency Key`とする。Anonymous RequestのKeyは400で拒否し、Tenant ScopeはPhase 20へ送る。
...
1. Phase 19 Decision、Specification、Failure Matrix
