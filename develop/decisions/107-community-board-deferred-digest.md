...
1. Digest ValueはISO 8601 Week Dateの`YYYY-Www`を受理し、UTCの月曜00:00以上、翌月曜00:00未満を集計する。形式だけでなく、そのISO Yearに実在するWeekかをDomainで検証する。
2. Digestは実行時点でApplication Tableに存在する対象週のPost数とComment数を集計し、件数から作る決定的なPlain Text Summaryを保存する。Post Title、Body、Preview、Comment本文は保存しない。
3. 保存済みDigestはImmutable Snapshotとする。後からPostがHard Deleteされても既存Digest Rowは書き換えず、再生成時はその時点で存在するRowだけを集計する。
4. 成功したGenerate Requestごとに新しいDigest ID／Rowを作り、同じRequested User／Weekの複数Rowを許可する。P17-007では暗黙IdempotencyやUpsertを追加しない。
5. Retry表示用にApplication-owned `DigestAttemptGate` PortをOperation実行境界へ置く。ProductionはNo-op、Development／Test Compositionだけが明示Configuration時にAttempt 1で`RetryableException`を投げる。Failure FlagをOperationValue、Generated Contract、Browserへ追加しない。
6. Proposed BFF Progress Contractを採用する。Initial `.status()`、有限Abort付き`.wait()`、Deadline後のSafe Snapshot、Typed Outcomeから`ShowDigest`への固定RouteをSvelteKit Server-only境界で実装する。
...
- `board_digests`はUUID Primary Key、Requested User ID、Week、Content、Post Count、Comment Count、Created Atを持ち、Requested User／WeekのUnique Constraintを持たない。
- Digest Contentは削除済みPost／Commentの本文やTitleを復元できない。既存Snapshotの集計値は生成時点の履歴として残る。
- `DigestService`がWeek解釈、UTC範囲、集計、Summary生成、ID／時刻、保存、Owner付き取得を所有する。OperationはActor、Attempt Gate、DomainService、Outcome／Rejection変換だけを扱う。
- Semanticに存在しないISO Weekは安定したApplication Rejectionへ変換し、Raw Date Parser Errorを返さない。
- Attempt 1 Failure AdapterはBuild／Bootstrap時の明示Configurationで選び、Production DefaultをNo-opとする。Config値をRequestごとに読まない。
- Status AuthorizerはCurrent UserとOrigin Userの一致を要求し、Unknown／Non-owner／Expiredを同じSafe Not Found Surfaceへ閉じる。
- BrowserへOperation IDを渡すが、Credential、Actor ID、Attempt ID、Internal URL、Journal／Raw Errorを渡さない。
- 同一User／Weekの重複抑止はPhase 18 Idempotency KeyのConcrete Journeyへ残す。
