...
- 旧`blackops:*` Project CLI Aliasと予約の削除
...
3. `1.1.0`では旧`bin/blackops` Entrypointを互換対象とせず、旧`blackops:*` Project CLI Aliasを削除する。新SkeletonはProject Root `blackops`だけを公式Entrypointとする。
