...
ユーザーが作成するアプリケーションにInternalは不要です。意味不明です。
Infrastructure/BlackOpsも意味不明です。BlackOpsという名前をユーザーのアプリケーションに矯正するものではありません。
CのFeatureベースが良さそうです。
...
## Decided Installed Tree

回答を反映した `composer create-project` 直後の概略は次のとおりになる。
...
1. Installed Applicationの公式SkeletonはFeature-first構造とする。
2. Operation Definition、Value、Handler、Outcome、Responderは `app/Feature/<Feature>/<Action>/` にまとめる。
3. Skeletonへ `Internal` Directoryを設けない。HTTPから直接開始しないOperationも、それが属するFeatureへ置く。
4. `app/Infrastructure/` はApplicationが必要な技術実装を置く任意のDirectoryとし、`BlackOps` というFramework名のSubdirectoryを要求または推奨しない。
5. FrameworkはFeature Directoryを実行要件として強制しない。ApplicationはOperation Providerまたは設定により独自の配置を使用できる。
6. `bootstrap/app.php` は `BlackOps\Application\Application` のBuilderから共有Application Objectを作成して返す。
7. `public/index.php` と `bin/blackops` は同じ `bootstrap/app.php` を読み、Application Objectから各Process Boundaryを起動する。
8. Application ObjectはProcess Entrypoint専用のComposition Rootであり、業務Codeから利用するService Locatorにはしない。
9. Starter FeatureとしてInline WelcomeとDeferred Reportを含める。Sensitive Mask、Retry、Build、Migration、Worker、Retentionの実行導線も提供する。
10. SkeletonはPHP 8.5 CLI／Composer、FrankenPHP、PostgreSQLを含むDocker Compose Quickstartを持つ。
11. DefaultのCompose起動はHTTPとPostgreSQLに限定し、WorkerとMaintenance Schedulerは明示的なCLI実行またはProfileで起動する。
12. Dotenv読込はSkeletonが所有し、Frameworkは特定Dotenv実装へ依存しない。実Process Environmentを優先し、`.env` はVersion管理せず `.env.example` を提供する。
13. Build ArtifactとRuntime Logは `var/build/`、`var/log/` へ分離し、生成物をVersion管理しない。
14. D011の「FrameworkがApplication全体のDirectory構造を強制しない」「FeatureとAction単位で関連Fileを近くへ置く」という原則は維持する。
15. D011の公式Skeleton未決定状態と `Application/Operation/Internal/` 推奨は、本DecisionのFeature-first Skeletonによって更新する。
...
- Featureを削除すると関連するOperation、Value、Handler、Outcome、Responderをまとめて削除できる。
- HTTP、Deferred、内部起点という実行経路をApplication Directory名へ固定せず、Operation MetadataとConfigurationで表現できる。
- Framework名をApplication NamespaceやDirectoryへ持ち込まず、Application自身の概念を中心に構成できる。
- HTTP、Console、Workerが同じApplication Objectを使うため、Provider、DB Schema、Artifact Pathの構成差異を防げる。
- Install直後からInlineとDeferredの両方をLocal Runtimeで検証できる。

### Constraints

- Feature-firstは公式Skeletonの推奨であり、Framework Scannerが `app/Feature/` を固定PathとしてHard Codeしてはならない。
- Application ObjectのPublic APIはSemVer互換性の対象となり、Internal Runtime型をSignatureへ露出できない。
- Starter Featureは一つのFeature Directory単位で安全に削除できなければならない。
- DotenvはApplication Dependencyであるため、Framework単体Testは `.env` の存在を前提にできない。
- WorkerとMaintenanceをDefault Compose Processに含めず、明示起動の安全性を維持する必要がある。
