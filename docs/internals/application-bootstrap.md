...
- `ApplicationConsoleKernel`: Symfony Applicationと10個のLazy Command Descriptorの内部Composition
...
`make:operation`も同じLazy Descriptorとして登録する。Kernel構成時はOperation SourceまたはFramework Stubを読まず、Command実行時だけApplication Base PathとFramework Package内StubをGeneratorへ渡す。Application CommandはGenerator名を上書きできない。
