...
The production runtime composer uses the loaded artifacts to build the inline dispatcher and HTTP request handler. Its HTTP route registry constructs FastRoute directly from the Compile済みDispatcher Data, so request handling performs no Attribute exploration or route compilation. Application-owned runtime resources such as the clock, journal writer, response factory, and stream factory are provided explicitly by the composition root.
