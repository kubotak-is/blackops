...
Application build automatically registers every compiled handler class as a public autowired service after applying application service providers. Existing definitions, aliases, and service instances are preserved, so an explicit application binding wins. Self-handled and separate handlers follow the same rule; interface dependencies remain the responsibility of an application service provider.
...
For Self-handled metadata, HTTP composition, deferred execution, and lease recovery resolve the handler from this compiled container and reuse that exact object as the operation definition. Separate-handler operation definitions continue to use their no-argument construction path.
