...
`basis_at` はRetention期間を計算する基準時刻である。Transport PayloadではTerminal Stateへ到達した時刻、Outcomeでは完了時刻、Dead LetterではDead Letterへ移動した時刻を使う。
...
Canonical Journalの物理削除は、削除順序とStorage境界を後続Taskで固定してからPlannerへ接続する。
...
Canonical JournalとSystem Log配送は後続Taskで接続する。
