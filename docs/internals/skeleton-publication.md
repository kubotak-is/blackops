...
| Repository | `https://github.com/kubotak-is/blackops` |
| Default branch | `main` |
...
1. GitHubへ空のPublic `kubotak-is/blackops` Repositoryを作成する。READMEやLicenseを初期生成せず、最初の`main`をWorkflowにPublishさせる。
2. Repository専用Ed25519 Key PairをRepository外の一時Directoryに作成する。
3. Public KeyをDistribution RepositoryのWrite-enabled Deploy Keyとして登録する。
4. Private KeyをFramework Main RepositoryのActions Secret `SKELETON_DEPLOY_KEY`へ登録する。
5. Local Private Keyを削除し、Working TreeとGit Historyへ混入していないことを確認する。
6. Packagistの`blackops/skeleton`を`https://github.com/kubotak-is/blackops`へ接続し、GitHub Tag連携を有効にする。
