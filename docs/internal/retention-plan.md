...
`basis_at` はRetention期間を計算する基準時刻である。Transport PayloadではTerminal Stateへ到達した時刻、JournalではOperation IDごとの最新 `occurred_at`、Outcomeでは完了時刻、Dead LetterではDead Letterへ移動した時刻、Idempotency Recordではrecordの`created_at`を使う。
...
Policyは5対象すべての期間を持つ。Idempotency Record期間はConfig／CLIで省略でき、既存4対象の最長値へ解決してからPlannerへ渡す。Policy未設定時の警告やPurge停止はConfig/Manifest/CLI側で扱う。
...
Idempotency Record候補はterminal rowの`created_at`が期限を過ぎたものを返す。`expires_at`を過ぎてもRecordはPurgeまでKeyの再利用を防ぐ。
...
## Idempotency Record Delete

`PostgreSqlIdempotencyRetentionDeleteService` はPlan内の `idempotency_record` 候補だけを処理する。Delete文でterminal state、Planのeligible time、Active Holdを再確認し、Plan後にHoldが置かれたRecordは安全側へSkipする。削除とPayloadなしのPurge Auditは同じTransactionへ含める。
...
既存4対象の期間は明示Optionで受け取り、Idempotency Record期間は`--idempotency-record-days`で上書きできる。省略時は既存4対象の最長値を使う。

`retention:purge` は注入済みのPlanner／Purge Serviceを使ってDry RunまたはConfirmを実行し、ApplicationのRetention Configurationは既存4対象と任意のIdempotency Record期間を同じAccepted Policyへ解決する。
