...
The installed Quickstart demonstrates this boundary with one policy shared by the inline Welcome and deferred Report operations. HTTP authentication creates the `quickstart-user` authorization actor, while worker composition replaces only execution with `quickstart-worker-1` / `system`. The worker evaluates the same compiled policy before each Report attempt. The sample header token never enters the operation value or transported context; the report's separate `recipientEmail` field remains the sensitive business-data projection example.

`ApplicationWorkerComposer` writes lifecycle records to the canonical PostgreSQL journal but does not construct or flush the HTTP process's observed JSONL pipeline. Worker attempt, retry, rejection, and completion evidence must therefore be queried from the canonical journal. The Quickstart JSONL file contains only records projected by the HTTP runtime; it is not a combined cross-process lifecycle log.
