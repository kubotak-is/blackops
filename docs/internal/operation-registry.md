...
Metadata and HTTP route compilation operate on definition class names and Reflection; they do not instantiate definitions. Typed Self-handled compilation requires a public non-static `handle` accepting one concrete OperationValue, optionally a required `ExecutionContext`, and returning one concrete Outcome or `void`. Value and Outcome are inferred; matching Accepts and Returns remain migration-compatible. This permits Self-handled definitions with required constructor dependencies. Runtime resolves a Self-handled definition from the compiled handler container and uses that same instance as both definition and handler. Legacy Self-handled and Separate `OperationHandler` definitions retain their existing Envelope and OperationResult contract.
