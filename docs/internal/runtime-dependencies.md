...
BlackOpsのRuntime依存をComposerで固定し、PHP 8.5上で品質検査できるBaselineを構築する。確定仕様の正本は `develop/spec/` である。この文書は各Packageの採用理由、用途、Version Constraint、PHP 8.5互換性の確認結果を記録する。
...
| Package | Constraint | Lock Version | PHP Requirement | Spec |
| --- | --- | --- | --- | --- |
| `doctrine/dbal` | `^4.4` | 4.4.3 | `^8.2` | Spec 35 PostgreSQL Transport Schema |
| `doctrine/migrations` | `^3.9` | 3.9.7 | `^8.1` | Spec 35 PostgreSQL Transport Schema |
| `symfony/stopwatch` | `^7.4` | 7.4.8 | `>=8.2` | Spec 35 PostgreSQL Transport Schema |
| `doctrine/event-manager` | transitive | 2.1.1 | `^8.1` | Doctrine Migrations |
| `psr/cache` | transitive | 3.0.0 | `>=8.0.0` | Doctrine DBAL |

採用理由：Deferred Transport、Claim、Fencing、Canonical Journal、MigrationをPostgreSQL上で安全に扱うため、Database接続、Transaction、低レベルSQL実行にはDoctrine DBALを採用する。Framework SchemaのVersioningとDeployment時の明示的な適用にはDoctrine Migrationsを採用する。Doctrine ORM、Symfony full-stack、DoctrineBundle、DoctrineMigrationsBundleは必須依存にしない。Symfony ComponentのMajor Version不整合を避けるため、Doctrine Migrationsが利用するStopwatchは7.4 LTS系列へRoot Constraintで固定する。
...
Deptracは`deptrac/deptrac` `4.7.1`をPHP 8.5対応のexact development dependencyとして固定する。4.6.2のvendor parser停止を無視せず、Architecture Rules／Layerを変更せずにProject Graph全体を解析する。更新は`--with-all-dependencies --minimal-changes`で当該Packageだけへ限定する。
