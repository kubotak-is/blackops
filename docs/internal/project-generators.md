...
Project Generatorは、Project Rootに置く薄い`blackops` EntrypointからFramework Package内のCommandとStubを遅延利用する。Application BootstrapやSkeletonはCommand実装とStubを複製しない。
...
2. Project Rootの`blackops`と生成済みSourceのSHA-256を保存する
...
更新中に不変性を検証する公式EntrypointはProject Rootの`blackops`だけであり、旧`bin/blackops`は互換対象としない。
