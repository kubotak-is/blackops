...
The generated artifacts contain scalar values, arrays, class names, synthetic runtime service definitions, and Framework-owned proxy classes. They must not contain credentials, resolved database connection parameters, tokens, environment secrets, closures, or live service instances. `DatabaseManager`, the default DBAL `Connection`, and the internal transaction runtime are synthetic definitions. HTTP and deferred worker composition set them before resolving application handlers, policies, or middleware. The transaction runtime receives the same execution-scope provider used by inline or deferred operation execution. Container compilation validates transactional connection names against the accepted configuration snapshot but does not connect to a database.

Application-aware `build:compile` inspects non-synthetic Symfony service definitions after providers, handlers, authorization policies, and HTTP middleware have been registered. Definitions with `#[Transactional]` or `#[AfterCommit]` are validated and emitted through the Framework-owned proxy profile. The common `proxy-profiles/<build-id>-<content-hash>/` unit records the Framework profile and references the matching `framework-proxies/<build-id>-<input-hash>/` unit. Generation validates source, Definition, manifest, and file hashes in a unique staging directory, then atomically publishes the complete unit; a failed build leaves the previous complete artifact available or publishes no new container.

The dumped Container loads `ProxyProfileArtifactLoader` and `FrameworkProxyProfileLoader`, which validate the common unit, Build ID, profile, manifest hash, inventory, class map, file hashes, and symlink boundary before generated code is loaded. Runtime requires only files listed by the matching manifest; it does not scan application source, regenerate a proxy, or fall back to another profile. Proxy identities include the Build ID or an equivalent immutable directory identity, and stale files are removed only after successful publication. A direct `new` expression still creates the original class and bypasses interception; only instances constructed from the compiled Symfony container use the generated proxy. General service transaction bindings resolve to the internal required-transaction interceptor, and after-commit bindings resolve to the shared callback queue. Operation transaction bindings remain pass-through because the compiled operation metadata drives the fixed lifecycle after authorization; this prevents a second transaction around `handle()`.
