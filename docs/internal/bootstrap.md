...
The generated tree contains `types.ts`, `client.ts`, `manifest.json`, and one module per routed Operation. `manifest.json` records generator schema version 2, the Application Build ID, and the canonical frontend-contract hash without timestamps or source paths. Ownership checks accept the known version 1 marker so a tree produced before typed fetch can be replaced, while newly written temporary trees must validate as the current version.
