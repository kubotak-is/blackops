...
| `BlackOps\Application\ApplicationBuilder` | final class | Environment、Config、Providerを組み立てる | `withEnvironmentFile()`、`withEnvironment()`、`withConfiguration()`、`create()`を呼ぶ |
...
| `BlackOps\Core\EphemeralOutcome` | marker interface | HTTPへ一度だけ返す非永続化Outputを示す | Credential Responseのfinal readonly DTOで実装する |
| `BlackOps\Core\OutcomeData` | marker interface | Outcome内のReadonly Nested Response Shapeを示す | `final readonly`なNested DTOで実装する |
...
| `BlackOps\Core\OperationResult` | final readonly class | Completed／Rejected Result、任意のOperation ID、Replay markerを保持する | 拒否時は`rejected($reason, $operationId)`、Replayは`completed($outcome, $operationId)->asReplayed()`で相関IDを残す |
| `BlackOps\Execution\Dispatcher` | interface | Operationを実行してResultを返す | 独自入口では任意の`ActorContext`とOptional `IdempotencyKey`を末尾へ渡す |
| `BlackOps\Execution\Operations` | interface | Framework管理TransactionへDeferred child OperationをClass＋Valueで登録する | Application CoordinationへConstructor Injectionする |
| `BlackOps\Execution\DispatchReceipt` | final readonly value object | child Operation IDとUTC dispatch時刻を保持する | `operationId()`／`dispatchedAt()`を読む |
...
| `BlackOps\Core\Attribute\ConsoleCommand` | attribute class | OperationをBlackOps CLIへ明示公開する | Canonical Command名と任意Descriptionを指定する |
...
| Namespace／Type | Kind | Purpose | Typical Use |
| --- | --- | --- | --- |
| `BlackOps\Http\SapiRuntime` | final class | Framework-owned Classic／FrankenPHP Worker SAPI実行境界 | Public Entrypointから`run($application)`または`runWorker($application)`を呼ぶ |
...
| Namespace／Type | Kind | Purpose | Typical Use |
| --- | --- | --- | --- |
| `BlackOps\Auth\Session\SessionIdentityProvider` | interface | Opaque Identity IDを現在有効なActorへ解決する | ApplicationのUser／Account Stateから`ActorRef`を返す |
| `BlackOps\Auth\Session\SessionConfiguration` | final readonly class | Absolute TTLとLast-used Touch Intervalを固定する | Default 8時間／5分をApplication起動時に変更する |
| `BlackOps\Auth\Session\SessionManager` | interface | SessionのIssue／Rotate／Revoke／Cleanupを提供する | Login、Token Rotation、Logout、Retention Jobから呼ぶ |
| `BlackOps\Auth\Session\RawSessionToken` | final readonly value object | 発行直後のRaw Tokenを明示取得に限定する | `IssuedSession::token()->reveal()`で一度だけCredential化する |
| `BlackOps\Auth\Session\IssuedSession` | final readonly value object | Token、Issued At、Expires Atだけを返す | Login／Rotation ResponseからApplication所有Credentialへ渡す |
| `BlackOps\Auth\Session\InvalidSessionException` | exception class | RotationのInvalid Stateを同じSafe Messageへ閉じる | Malformed／Unknown／Expired／Revokedを区別せず扱う |
| `BlackOps\Auth\Session\SessionCookieName` | final readonly value object | Application所有のCookie名を検証する | Cookie AdapterのConstructorへ渡す |
| `BlackOps\Auth\Session\BearerSessionAuthenticator` | final readonly class | Bearer CredentialをSessionとActorへ解決する | API向け`HttpAuthenticator`として登録する |
| `BlackOps\Auth\Session\CookieSessionAuthenticator` | final readonly class | 指定CookieだけをSessionとActorへ解決する | Browser AdapterのCredential抽出に使う |
| `BlackOps\Auth\Session\SessionServiceProvider` | final readonly service provider | Session Coreと選択AdapterをCompiled ContainerへOpt-in登録する | `bearer()`または`cookie()`を`services`へ追加する |
...
| Namespace／Type | Kind | Purpose | Typical Use |
| --- | --- | --- | --- |
| `BlackOps\Idempotency\IdempotencyKey` | final readonly value object | CallerのPrintable ASCII Keyを検証し、Raw Valueを公開せずOpaque Hashへ変換する | Network Retryで同じKeyを再利用するとき`hash()`をFramework入口へ渡す |
| `BlackOps\Idempotency\IdempotencyKeyHash` | final readonly value object | Version付きSHA-256 Digestを保持する | `ExecutionContext::idempotencyKeyHash()`からOpaque相関を読む |
...
| Namespace／Type | Kind | Purpose | Typical Use |
| --- | --- | --- | --- |
| `BlackOps\Status\OperationStatusQuery` | interface | Operation IDから認可済みStatusを読む | ApplicationのStatus Adapterへ注入する |
| `BlackOps\Status\OperationStatusResult` | marker interface | Found／Unavailable／Expiredを束ねる | Query Resultを分岐する |
| `BlackOps\Status\OperationStatusFound` | final readonly value object | 取得できたStatusを保持する | `status()`から7 Stateを読む |
| `BlackOps\Status\OperationStatusUnavailable` | final readonly value object | Unknown／Deny／完全Purgeを区別せず表す | Safe 404へ変換する |
| `BlackOps\Status\OperationStatusExpired` | final readonly value object | 認可済みRetention期限切れを表す | Safe 410へ変換する |
| `BlackOps\Status\OperationStatus` | final readonly value object | StateごとのAttempt、Retry、Outcome、Safe Errorを保持する | Named Constructorで7 Stateを扱う |
| `BlackOps\Status\OperationStatusState` | enum | Publicな7 Lifecycle Stateを定義する | Status Resultを分岐する |
| `BlackOps\Status\OperationStatusError` | final readonly value object | Rejected／Failed／Dead LetterのSafe Errorを保持する | Category／安定Codeだけを返す |
| `BlackOps\Status\OperationStatusAuthorizer` | interface | Status参照判断をApplicationへ委譲する | Service ProviderからApplication PolicyをBindingする |
| `BlackOps\Status\OperationStatusAuthorizationRequest` | final readonly value object | Operation ID／Type、Current／Origin Actorを渡す | PayloadやOutcomeを読まずPolicy評価する |
| `BlackOps\Status\OperationStatusAuthorizationDecision` | final readonly value object | Allow／Denyを表す | Authorizerから判断を返す |
| `BlackOps\Status\DenyOperationStatusAuthorizer` | final readonly class | 未Binding時のFail-closed既定値 | Status Detailを常に拒否する |
| `BlackOps\Status\Exception\OperationStatusQueryException` | exception class | Authorization／Storage／Decode／Integrity失敗を安全なCodeへ分類する | Adapter境界の500として扱う |
...
| `BlackOps\Core\Execution\Deferred` | final readonly class | Durable受付後のWorker実行を表すStrategy Identity | `#[Deferred]`で選ぶ |
...
| Namespace／Type | Kind | Purpose | Typical Use |
| --- | --- | --- | --- |
| `BlackOps\Outbox\TransactionalOutbox` | interface | Framework管理TransactionへDeferred child Operationを登録する低Level互換API | 既存Integration互換用。新規Application Coordinationは`BlackOps\Execution\Operations`をConstructor Injectionする |
...
| Namespace／Type | Kind | Purpose | Typical Use |
| --- | --- | --- | --- |
| `BlackOps\Core\Retention\RetentionTarget` | enum | Payload／Journal／Outcome／Dead Letter／Idempotency Record対象を表す | PolicyとPlanを分類する |
...
| `BlackOps\Core\Retention\RetentionPurgeResult` | final readonly value object | 対象別削除件数を保持する | Purge結果とAuditを表示する。`idempotencyRecordsDeleted()`でIdempotency Record件数を読む |
