...
`examples/mvp` は、BlackOps MVPのInline／Deferred Vertical Sliceを一つの小さなApplicationとして示す。
...
Application buildではSample Provider ConfigからOperation Manifest、HTTP Manifest、DI Containerをcompileする。
...
php bin/console blackops:build:compile \
  ...
  --application-build-id=mvp-sample
```
