...
Scheduled Application Operationは、公開済みExperimental Stable `1.2.1`で提供するCapabilityです。[Releases](mvp-status.md)でApplication-owned責務とFrameworkの境界を確認してから、専用のProject Rootで試してください。
...
次の例は公開済みExperimental Stable `1.2.1`のCanonical Authoringです。`#[Deferred]`を外すとInlineになります。
