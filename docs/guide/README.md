...
[Quickstart and Skeleton](mvp-sample.md)は空DirectoryからComposer Install、Inline Request、Deferred受付、Worker実行までを一Pageで案内します。初めて使う場合は[Install](installation.md)で前提を確認してから進んでください。
...
4. [Directory](directory-structure.md): Applicationが所有する構成をつかむ
...
設計から理解する場合は[What's BlackOps](why-blackops.md)、[Core Concepts](core-concepts.md)、[Lifecycle](operation-lifecycle.md)、[Journal](journal.md)の順に進みます。必要なPageだけを探す場合は、左SidebarまたはSearchから[Authoring](operations.md)、[Inline and Deferred](execution.md)、[Testing](testing.md)、[Deployment](deployment.md)、[Troubleshooting](troubleshooting.md)へ進めます。
