...
Deferred Workerはoriginとauthorizationを維持し、AttemptごとにexecutionだけをWorker System Actorへ置き換えます。Worker Actorは`execution.worker.id`をID、`system`をTypeとして持ちます。この分離により、User起点の処理をWorkerが実行しても、元Userの権限をWorker自身の権限へ強化しません。

受付時にActorがないOperationでも、Worker Attemptでは`origin=null`、`authorization=null`、`execution=<worker ID>/system`のActor Contextを持ちます。Policy付きOperationはauthorization ActorがないためHandlerを実行せずRejectedになります。PolicyなしOperationはWorker Actorを監査記録へ残して通常どおり実行します。

RetryとLease Expired Recoveryも受付時のorigin／authorizationと現在のWorker execution Actorを復元します。Retry先のWorker IDが変わればexecution Actorも変わるため、Journalから「誰が要求し、誰の権限で評価し、どのWorkerが各Attemptを実行したか」を区別できます。
