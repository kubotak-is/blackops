...
| Full-stack Browser | Application-owned Authentication、SvelteKit BFF、Inline／Deferred UI、Sensitive Boundary、Accessibility | [BlackOps Board](community-board.md) |
...
QuickstartはFrameworkの最短Contractを実HTTPへ接続します。BlackOps BoardはそのContractをApplication-owned Authentication、Domain／Infrastructure、SvelteKit Same-origin BFF、Deferred Progress UIへ広げたReference Applicationです。[BlackOps Board Guide](community-board.md)では、Clean Installと個別Consumer、実Browser E2Eの使い分けを確認できます。
