...
`build:compile`は`--proxy-profile=ray`（既定）または`framework`を選択し、Containerと同じBuild IDの`proxy-profiles/<build-id>-<content-hash>/`へ完全なProfile Artifact Unitを発行します。Unitの`manifest.json`、Profile、Build ID、Content Hash、File Inventoryは一体で配備してください。Framework Profileでは同じUnitがFramework Proxy ManifestのDirectory／Hashを参照します。前回の完全Unitは保持し、部分コピーや別Profileの混在を起動時に許可しません。
...
`#[Transactional]`のDefaultとNamed ConnectionもこのSnapshotに対してBuild時に検証します。この検証はConnection Nameだけを使い、Databaseへの接続やCredentialのBuild Artifactへの保存を行いません。Proxy Artifactは追加Config Keyを使わず、`build.container`と同じDirectoryに共通の`proxy-profiles/` Unitとして生成されます。Ray ProfileではUnitがProxy File Inventoryを保持し、Framework ProfileではUnitが一致する`framework-proxies/` Directory／Manifest Hashを参照します。Container、各Manifest、共通Profile Unit、Framework参照Unitを同じBuild IDの完全なRelease組として配備してください。
