...
Generated Typeは認証、認可、暗号化、Access Control、Retentionを代替しません。`OperationCallOptions.headers`へSession／Token等を呼出単位で注入し、Global Mutable Clientへ保存しないでください。ApplicationはCORS、CSRF、TLS、Browser Storage、Token Rotation、Frontend Source Map／Build Artifactの公開範囲を管理します。`config/frontend.php`へCredentialやBase URLを保存しません。
